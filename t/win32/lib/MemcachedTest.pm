...
sub supports_udp {
    my $output;
     if ($^O == "MSWin32") {
         open(NETPROC, "$builddir/memcached -h");
         $output = <NETPROC>;
         return 0 if $output =~ /^memcached 1\.1\./;
         return 1;
     }
     else {
        $output = `"$builddir/memcached-debug -h"`;
        return 0 if $output =~ /^memcached 1\.1\./;
        return 1;
     }
}
...
sub new_memcached {
	...
	$args = "memcached " . $args;
        Win32::Process::Create($obj, "$exe", $args, 1, NORMAL_PRIORITY_CLASS, ".") || 
		croak("memcached startup failed on Windows\n");
	...
	return Memcached::Handle->new(pid  => $pid,
				      conn => $conn,
